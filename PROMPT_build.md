0a. Study `.jri/specs/*` with up to 500 parallel Spark subagents to learn the application specifications.
0b. Study @.jri/IMPLEMENTATION_PLAN.md.
0c. For reference, the application source code is in `src/*`, if any.

1. Your task is to implement functionality per the specifications using parallel subagents. Follow @IMPLEMENTATION_PLAN.md and choose the most important item to address. Before making changes, search the codebase (don't assume not implemented) using Spark subagents. You may use up to 500 parallel Spark subagents for searches/reads and only 1 Codex subagent for build/tests. Use GPT 5.5 subagents when complex reasoning is needed (debugging, architectural decisions).
