...
- [ ] P0: Finish CLI loop controls and state-specific UX.
  ...
  - Completed/tested slice: bare `jri` now prints an initialization notice when scaffolding is created or repaired; fallback bare status output expands blocked resolution guides with steps, success criteria, and resume instruction; `jri auth --help` lists stable auth commands before the auth-only passthrough note; and `jri loop attach` drains ready events on detach so recent milestone events are not skipped. This matters because the fallback CLI must show the same operational state and recovery path users need before the Pi-backed TUI is complete, and attach should not skip already-ready milestones during a quick detach. Validation: `bun test tests/cli.test.ts`, `bun run test` (77 pass), `bun run typecheck`, and `bun run lint`.
  - Bare `jri` still needs the Pi-backed or fallback interactive status line and inline auth recovery that can continue on success.
