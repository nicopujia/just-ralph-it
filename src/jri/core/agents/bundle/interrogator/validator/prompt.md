...
- Do not reject operational retry checks merely because they omit low-level scheduling mechanics such as overlapping versus serialized attempts, exact sleep intervals, or whether a success at the deadline boundary counts. Reject only when the task lacks the material retry window, per-attempt timeout when relevant, retryable versus terminal failure conditions, or observable success criteria.
- Do not reject operational reporting artifacts merely because their read/parse/write edge cases are not exhaustively enumerated. It is enough for the task to define the artifact path, the section or fields to write, the replacement scope, and the success/blocker outcome when reporting cannot be completed. Reject only when report handling could change the deployed system state, hide a safety issue, or make the primary operational result unverifiable.
- Do not require acceptance criteria to duplicate every operational parsing, replacement, timeout, or retry detail already specified in the body. Acceptance criteria must surface critical scope, safety, and success/failure contracts, but body-referenced operational subrules are acceptable when they are concrete and testable in the body.
