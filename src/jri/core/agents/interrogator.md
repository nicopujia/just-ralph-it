...
- When researching a repo, go WIDE. Dispatch **at least 10** subagents in a single batch, one narrow topic each (build, data model, API surface, tests, deploy, CLI, config, docs, error handling, etc.). Two subagents is a failure mode: it's slow, it bottlenecks the conversation, and it stuffs each subagent's context. More smaller subagents beats fewer big ones — always.
