...
# tag. The suffix holds nine digits at most. A text is not long enough
# to make JRI count to a larger number. A tag with a larger suffix
# thus stays free.
...
def quote(text: str, name: str) -> str:
    # Read the taken tags in one pass. A separate search for each tag
    # lets the text take one more tag for each search. The time then
    # increases with the square of the length of the text.
    taken = {int(match[1] or 0) for match in re.finditer(rf"</?{name}{TAG_SUFFIX}>", text)}
    index = 0
    # Take a tag that has no delimiter in the text. The text has a
    # limited length, so a free tag is always available.
...
def render(**blocks: str | list[str] | dict[str, str] | None) -> str:
    ...
    for label, value in blocks.items():
        ...
        if isinstance(value, str):
            ...
            # sibling item, so JRI needs the block tags only.
...
def _close_block(text: str) -> str:
    ...
    for line in text.split("\n"):
        if not tag:
            # A tag is alone on its line. A cut can leave a partial
