"""Black-box smoke tests for the user-facing JRI CLI contract.
...
from tests.support.cli import CliHarness, CliRun


def test_help_does_not_require_model_credentials(
    credentialless_cli: CliHarness,
) -> None:
    """The CLI help works without model credentials."""
    result = credentialless_cli.run_help()
...
def test_invalid_option_does_not_initialize_project(
    ...
    result = credentialless_cli.run(cwd=tmp_path, args=("--not-a-real-flag",))
...
def test_interactive_run_requires_selected_provider_credentials(
    ...
    result = credentialless_cli.run(cwd=tmp_path)
...
def test_force_recreates_existing_jri_state(
    ...
    result = cli.run(cwd=tmp_path, args=("--force",))
...
def test_first_interview_turn_records_visible_response(
    ...
    result = cli.run(cwd=tmp_path, input_text=first_turn_input)
...
def test_early_just_ralph_it_keeps_interview_open_without_commit(
    ...
    cli.initialize_git_repo(tmp_path)

    result = cli.run(cwd=tmp_path, input_text=early_trigger_input)
...
def test_mvp_happy_path_finalizes_and_commits_jri_files(
    ...
    result = cli.run(cwd=tmp_path, input_text=mvp_happy_path_input)
