...
def _write_graph_node(
    repo: Path, semantic_path: str, *, title: str = "Node", state: str = "active", archive_reason: str | None = None
) -> Path:
...
def test_status_shows_counts_and_human_tasks(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    write_task(
        git_repo, status="todo", slug="human-task", title="Human task", priority=1, assignee="Human", body="do it"
    )
    write_task(
        git_repo, status="todo", slug="ralph-task", title="Ralph task", priority=0, assignee="Ralph", body="do it"
    )
    write_task(git_repo, status="done", slug="finished", title="Finished", priority=2, assignee="Ralph", body="done")
    write_task(
        git_repo, status="todo", slug="another-human", title="Another human", priority=0, assignee="Human", body="do it"
    )
...
def test_status_explains_actionable_human_blocker(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    write_task(
        git_repo,
        status="todo",
...
def test_status_explains_missing_nonhuman_dependency(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    write_task(
        git_repo,
        status="todo",
    ...
    assert rc == 0
    out = capsys.readouterr().out
    assert "Action needed: waiting for dependency upstream-task before blocked-ralph-task can start." in out


def test_status_no_human_tasks(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    write_task(
        git_repo, status="todo", slug="ralph-only", title="Ralph only", priority=0, assignee="Ralph", body="do it"
    )
...
def test_status_explains_fresh_ralph_task_as_work_on(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    write_task(
        git_repo,
        status="todo",
    ...
    assert rc == 0
    assert "Action needed: run `jri start` to work on fresh-ralph-task." in (capsys.readouterr().out)


def test_status_empty_project(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
...
def test_status_shows_ralph_running_state(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    ...
    service.state_store.start_attempt(AttemptState(number=1, task_slug="ralph-task", branch="ralph", started_at=1))
...
def test_status_shows_stale_tracked_ralph_process(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    process = subprocess.Popen(["sleep", "0"])
    process.wait(timeout=5)
    service = JriService(git_repo)
    service.state_store.save_process(loop_pid=process.pid, child_pid=None, log_path=None, detached=False)
    ...
    assert "Action needed: run `jri start --force` to recover interrupted Ralph state." in out


def test_status_shows_stale_doing_without_tracked_process(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    write_task(
        git_repo,
        status="doing",
...
def test_status_shows_human_tasks_across_tracked_states(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    """Human tasks in tracked states (todo, doing, done) are shown."""
    _init(git_repo)
    write_task(
        git_repo, status="todo", slug="human-todo", title="Human todo", priority=1, assignee="Human", body="todo"
    )
    write_task(
        git_repo, status="doing", slug="human-doing", title="Human doing", priority=2, assignee="Human", body="doing"
    )
    write_task(
        git_repo, status="done", slug="human-done", title="Human done", priority=3, assignee="Human", body="done"
    )
...
def test_status_rejects_in_place_mutation_of_lifecycle_task(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
...
def test_status_shows_graph_counts(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
    _write_graph_node(git_repo, "product", title="Product")
    _write_graph_node(git_repo, "product/old", title="Old", state="archived", archive_reason="Replaced")
...
def test_status_reports_invalid_graph_errors(git_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init(git_repo)
